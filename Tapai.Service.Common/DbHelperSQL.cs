...
namespace Tapai.Service.Common
{
    public abstract class DbHelperSQL
    {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        ...
                        object obj = cmd.ExecuteScalar();
                        //sw.Stop();
                        //System.IO.File.AppendAllText(@"d:\Websites\sql执行时间.txt", $"执行sql耗时：{sw.ElapsedMilliseconds}毫秒\r\n");
                        cmd.Parameters.Clear();
                        if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
                        {
                            return null;
        ...
        /// <param name="batchSize">一次执行的最大数据条数</param> 
        /// 创建者：蒋浩
        /// 创建时间：2017-11-14
        public static void SqlBulkCopyInsert(string strTableName, DataTable dtData, int batchSize = 100000)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    using (SqlBulkCopy bulkCopy = new SqlBulkCopy(trans.Connection, SqlBulkCopyOptions.KeepIdentity, trans))
                    {
                        ...
                        bulkCopy.BulkCopyTimeout = 3600;//最大时间为3600秒

                        bulkCopy.DestinationTableName = strTableName;//数据库中对应的表名  
                        ...
                        for (int i = 0; i < dtData.Columns.Count; i++)
                        {
                            ...
                            //bulkCopy  
                            bulkCopy.ColumnMappings.Add(dtData.Columns[i].ColumnName, dtData.Columns[i].ColumnName);
                        }
                        try
                        {
        ...
        public static bool SqlBulkCopyInsert<T>(List<T> data) where T : class, new()
        {
            bool flag = true;
            Type type = typeof(T);
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        using (var sqlbulk = new SqlBulkCopy(conn, SqlBulkCopyOptions.Default, trans))
                        {
                            sqlbulk.BatchSize = 100000;
                            sqlbulk.BulkCopyTimeout = 60;
                            sqlbulk.DestinationTableName = type.Name;
                            foreach (var item in props)
                            {
                                sqlbulk.ColumnMappings.Add(item.Name, item.Name);
                                Type columtype = item.PropertyType;
                                if (item.PropertyType.IsGenericType && item.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>))
                                {
                                    columtype = item.PropertyType.GetGenericArguments()[0];
                                }
                                dataTable.Columns.Add(new DataColumn(item.Name, columtype));
                            }

                            foreach (var m in data)
                            {
                                object[] row_data = new Object[props.Length];
                                int index = 0;
                                foreach (PropertyInfo item in props)
                                {
                                    row_data[index++] = item.GetValue(m, null);
                                }
                                dataTable.Rows.Add(row_data);
                            }
